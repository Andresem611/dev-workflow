...
> **Not the same as the `verify` skill.** `verify` runs pre-commit task-completion checks (type-check, lint, docs/stub drift, judge scoring). `verify-this` falsifies a *specific claim* with baseline/treatment evidence and returns a single verdict.
