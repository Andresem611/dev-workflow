...
### Layer 0 — Test Immutability Check (pre-wave gate)

Before any agent dispatch in this wave, verify that test files generated by DOCUMENT (Section 3.5) have not drifted.

**Command:**

```bash
node ${PLUGIN_ROOT}/../shared/tools/dev-pipeline-tools.js \
  verify-test-immutability <feature-dir> --plugin frontend
```

**Pass criterion:** parse stdout JSON; gate on `res.valid === true`.
...
- Failing here is cheap; failing later (after agents have written code that conflicts with tests) is expensive.

**Mode propagation:**
- Reduction: skip Layer 0 (Section 3.5 also skipped on Reduction; nothing to verify).
- Hold: run.
- Expansion: run + verify negative test files (`tests/T-NN.negative.test.ts`).
